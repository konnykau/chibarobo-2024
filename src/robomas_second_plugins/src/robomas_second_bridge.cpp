#include "robomas_second_plugins/robomas_second_bridge.hpp"
namespace robomas_bridge
{

    RobomasSecondBridge::RobomasSecondBridge(const rclcpp::NodeOptions &options) : Node("robomas_second_bridge", options)
    {
        rclcpp::on_shutdown([this]()
                            { this->onShutdown(); });
        can_rx_pub_ = this->create_publisher<robomas_plugins::msg::Frame>("robomas_can_rx2", 10);
        can_tx_sub_ = this->create_subscription<robomas_plugins::msg::Frame>("robomas_can_tx2", 10, std::bind(&RobomasSecondBridge::canTxCallback, this, _1));
        robomas_sub_ = this->create_subscription<robomas_plugins::msg::RobomasFrame>("robomas_frame2", 10, std::bind(&RobomasSecondBridge::robomasCallback, this, _1));
        robomas_target0_ = this->create_subscription<robomas_plugins::msg::RobomasTarget>("robomas_target2_0", 10, std::bind(&RobomasSecondBridge::robomasCallback1, this, _1));
        robomas_target1_ = this->create_subscription<robomas_plugins::msg::RobomasTarget>("robomas_target2_1", 10, std::bind(&RobomasSecondBridge::robomasCallback2, this, _1));
        robomas_target2_ = this->create_subscription<robomas_plugins::msg::RobomasTarget>("robomas_target2_2", 10, std::bind(&RobomasSecondBridge::robomasCallback3, this, _1));
        robomas_target3_ = this->create_subscription<robomas_plugins::msg::RobomasTarget>("robomas_target2_3", 10, std::bind(&RobomasSecondBridge::robomasCallback4, this, _1));
        robomas_target4_ = this->create_subscription<robomas_plugins::msg::RobomasTarget>("robomas_target2_4", 10, std::bind(&RobomasSecondBridge::robomasCallback5, this, _1));
        robomas_target5_ = this->create_subscription<robomas_plugins::msg::RobomasTarget>("robomas_target2_5", 10, std::bind(&RobomasSecondBridge::robomasCallback6, this, _1));
        robomas_target6_ = this->create_subscription<robomas_plugins::msg::RobomasTarget>("robomas_target2_6", 10, std::bind(&RobomasSecondBridge::robomasCallback7, this, _1));
        robomas_target7_ = this->create_subscription<robomas_plugins::msg::RobomasTarget>("robomas_target2_7", 10, std::bind(&RobomasSecondBridge::robomasCallback8, this, _1));
        ...
        RCLCPP_INFO(get_logger(), "RobomasSecondBridge is initialized.");

        initializeSerialPort(port_name_);
        asyncRead();
        handshake();
    }
    void RobomasSecondBridge::canTxCallback(const robomas_plugins::msg::Frame::SharedPtr msg)
    {
        if (!is_active_)
            return;
    ...
    void RobomasSecondBridge::robomasCallback(const robomas_plugins::msg::RobomasFrame::SharedPtr msg)
    {
        if (!is_active_)
            return;
    ...
    void RobomasSecondBridge::robomasCallback1(const robomas_plugins::msg::RobomasTarget::SharedPtr msg)
    {
        if (!is_active_)
            return;
    ...
    void RobomasSecondBridge::robomasCallback2(const robomas_plugins::msg::RobomasTarget::SharedPtr msg)
    {
        if (!is_active_)
            return;
    ...
    void RobomasSecondBridge::robomasCallback3(const robomas_plugins::msg::RobomasTarget::SharedPtr msg)
    {
        if (!is_active_)
            return;
    ...
    void RobomasSecondBridge::robomasCallback4(const robomas_plugins::msg::RobomasTarget::SharedPtr msg)
    {
        if (!is_active_)
            return;
    ...
    void RobomasSecondBridge::robomasCallback5(const robomas_plugins::msg::RobomasTarget::SharedPtr msg)
    {
        if (!is_active_)
            return;
    ...
    void RobomasSecondBridge::robomasCallback6(const robomas_plugins::msg::RobomasTarget::SharedPtr msg)
    {
        if (!is_active_)
            return;
    ...
    void RobomasSecondBridge::robomasCallback7(const robomas_plugins::msg::RobomasTarget::SharedPtr msg)
    {
        if (!is_active_)
            return;
    ...
    void RobomasSecondBridge::robomasCallback8(const robomas_plugins::msg::RobomasTarget::SharedPtr msg)
    {
        if (!is_active_)
            return;
    ...
    void RobomasSecondBridge::initializeSerialPort(const std::string port_name)
    {
    ...
    void RobomasSecondBridge::asyncWrite(const std::vector<uint8_t> data)
    {
        io_context_->post([this, data]()
                          { boost::asio::async_write(*serial_port_, boost::asio::buffer(data),
                                                     boost::bind(&RobomasSecondBridge::writeHandler, this, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred)); });
        return;
    }

    void RobomasSecondBridge::asyncWrite(const robomas_plugins::msg::Frame::SharedPtr msg)
    {
        // data structure
        /*
    ...
    void RobomasSecondBridge::asyncWrite(const slcan_command::Command command, const std::vector<uint8_t> data)
    {
    ...
    void RobomasSecondBridge::asyncWrite(const robomas_plugins::msg::RobomasFrame::SharedPtr robomasFrame)
    {
        // if (command == slcan_command::Normal)
        //     RCLCPP_ERROR(get_logger(), "asyncWrite(Command) can not use normal. you need to use asyncWrite(Frame)");
        ...
        */
        if (robomasFrame->motor < 8)
        {
            ...
            //  [
            //     ([7:4]: command | [3:3]: frameORtarget | [2:0]: motorID[1])
            //     , [7:7]: c610orc620 | mode[1]
            //     , temp[1]
            //     , kp[4]
            //     , ki[4]
            //     , kd[4]
            //     , limitie[4]
            //  ]
            raw_data[0] = 0b0011'0000 | 0x0 | robomasFrame->motor;
            raw_data[1] = robomasFrame->mode | (robomasFrame->c620 ? (std::uint8_t)1 << 7 : 0);
    ...
    void RobomasSecondBridge::asyncWrite(const robomas_plugins::msg::RobomasTarget::SharedPtr robomasTarget, uint8_t motor)
    {
    ...
    void RobomasSecondBridge::readingProcess(const std::vector<uint8_t> data)
    {
    ...
    bool RobomasSecondBridge::handshake()
    {
        rclcpp::WallRate rate(10ms);
    ...
    void RobomasSecondBridge::readOnceHandler(const boost::system::error_code &error, std::size_t bytes_transferred)
    {
        if (error)
        {
        ...
        RobomasSecondBridge::readingProcess(data);
    ...
    void RobomasSecondBridge::readHandler(const boost::system::error_code &error, std::size_t bytes_transferred)
    {
    ...
    void RobomasSecondBridge::asyncReadOnce()
    {
        ...
        boost::asio::async_read_until(*serial_port_, read_streambuf_, '\0',
                                      boost::bind(&RobomasSecondBridge::readOnceHandler, this, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));

        return;
    }

    void RobomasSecondBridge::writeHandler(const boost::system::error_code &error, std::size_t bytes_transferred)
    {
        if (error)
        {
    ...
    void RobomasSecondBridge::asyncRead()
    {
        // read and write functions can worl in the same time.
        // so, it is not necessary to use io_context_->post()      (this is a strand.)
        boost::asio::async_read_until(*serial_port_, read_streambuf_, '\0',
                                      boost::bind(&RobomasSecondBridge::readHandler, this, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
        return;
    }
...
RCLCPP_COMPONENTS_REGISTER_NODE(robomas_bridge::RobomasSecondBridge)